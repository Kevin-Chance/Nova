...
inline void client_handler(std::unique_ptr<simple_socket::SimpleConnection> conn, const std::string& fmu, const std::string& instanceName)
{
    ...
    std::unique_ptr<nova_fmi::slave> slave;
    std::vector<uint8_t> buffer(1024 * 32);
    auto op{nova_sim::proxy::opcodes::NONE};
    try {
        bool stop{false};
        while (!stop) {
            ...
            switch (op) {
                ...
                case nova_sim::proxy::opcodes::enter_initialization_mode: {
                ...
                case nova_sim::proxy::opcodes::exit_initialization_mode: {
                    const auto status = slave->exit_initialization_mode();
                    sendStatus(*conn, status);
                } break;
                case nova_sim::proxy::opcodes::step: {
                ...
                case nova_sim::proxy::opcodes::terminate: {
                    const auto status = slave->terminate();
                    sendStatus(*conn, status);
                } break;
                case nova_sim::proxy::opcodes::freeInstance: {
                    slave->freeInstance();
                    stop = true;
                    continue;
                }
                case nova_sim::proxy::opcodes::read_int: {
                    const auto flexVr = root[arg++].AsTypedVector();
                    std::vector<nova_fmi::value_ref> vr(flexVr.size());
                ...
                case nova_sim::proxy::opcodes::read_real: {
                    const auto flexVr = root[arg++].AsTypedVector();
                    std::vector<nova_fmi::value_ref> vr(flexVr.size());
                    for (auto i = 0; i < flexVr.size(); i++) {
                        vr[i] = flexVr[i].AsUInt32();
                ...
                case nova_sim::proxy::opcodes::read_string: {
                    const auto flexVr = root[arg++].AsTypedVector();
                    std::vector<nova_fmi::value_ref> vr(flexVr.size());
                    for (auto i = 0; i < flexVr.size(); i++) {
                        vr[i] = flexVr[i].AsUInt32();
                ...
                case nova_sim::proxy::opcodes::read_bool: {
                    const auto flexVr = root[arg++].AsTypedVector();
                    std::vector<nova_fmi::value_ref> vr(flexVr.size());
                    for (auto i = 0; i < flexVr.size(); i++) {
                        vr[i] = flexVr[i].AsUInt32();
                ...
                case nova_sim::proxy::opcodes::write_int: {
                    const auto flexVr = root[arg++].AsTypedVector();
                ...
                case nova_sim::proxy::opcodes::write_real: {
                    const auto flexVr = root[arg++].AsTypedVector();
                    const auto flexValues = root[arg++].AsTypedVector();
                ...
                case nova_sim::proxy::opcodes::write_string: {
                    const auto flexVr = root[arg++].AsTypedVector();
                ...
                case nova_sim::proxy::opcodes::write_bool: {
                    const auto flexVr = root[arg++].AsTypedVector();
                    const auto flexValues = root[arg++].AsVector();
