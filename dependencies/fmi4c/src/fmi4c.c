...
bool parseModelDescriptionFmi2(fmuHandle* fmu)
{
    ...
    fmu->fmi2.hasVectorVariables = false;
    ...
    if (modelVariablesElement) {
        for (ezxml_t varElement = ezxml_child(modelVariablesElement, "ScalarVariable"); varElement; varElement = varElement->next) {
            ...
            ezxml_t vectorElement = ezxml_child(varElement, "Vector");
            if (vectorElement) {
                fmu->fmi2.hasVectorVariables = true;
                var.datatype = fmi2DataTypeVector;

                var.vectorLength = 0;
                if (parseUInt32AttributeEzXml(vectorElement, "length", &var.vectorLength)) {

                    fmi2Real* tmp = (fmi2Real*)calloc(var.vectorLength, sizeof(fmi2Real));
                    if (!tmp) {
                        fprintf(stderr, "Error: failed to allocate memory for startVector (length=%u)\n", var.vectorLength);
                        var.startVector = NULL;
                        var.vectorLength = 0;
                        return false;
                    }
                    const char* startStr = ezxml_attr(vectorElement, "start");
                    if (startStr) {
                        if (strstr(startStr, ".bin")) {
                            char filepath[1024];
                            snprintf(filepath, sizeof(filepath), "%s/data/%s", fmu->unzippedLocation, startStr);
                            ...
                            size_t readCount = fread(tmp, sizeof(fmi2Real), var.vectorLength, fp);
                            fclose(fp);

                            if (readCount != var.vectorLength) {
                                fprintf(stderr, "Error: file %s has insufficient data (expected %u doubles, got %zu)\n",
                                    filepath, var.vectorLength, readCount);
                                free(tmp);
                                var.startVector = NULL;
                                var.vectorLength = 0;
                                return false;
                            ...
                            char* copy = strdup(startStr);
                            char* token = strtok(copy, " ");
                            uint32_t i = 0;
                            while (token && i < var.vectorLength) {
                                tmp[i++] = atof(token);
                                token = strtok(NULL, " ");
                            }
                            if (i > 0) {
                ...
                parseStringAttributeEzXmlAndRememberPointer(vectorElement, "quantity", &var.quantity, fmu);
                parseStringAttributeEzXmlAndRememberPointer(vectorElement, "unit", &var.unit, fmu);
            }
...
bool loadFunctionsFmi2(fmuHandle* fmu, fmi2Type fmuType)
{
    ...
    fmu->fmi2.getVector = (fmi2GetVector_t)loadDllFunction(dll, "fmi2GetVector", &ok);
    fmu->fmi2.setVector = (fmi2SetVector_t)loadDllFunction(dll, "fmi2SetVector", &ok);
    ...
    if (fmuType == fmi2CoSimulation) {
        ...
        fmu->fmi2.getVectorStatus = (fmi2GetVectorStatus_t)loadDllFunction(dll, "fmi2GetVectorStatus", &ok);
    }
...
fmi2Vector fmi2_getVariableStartVector(fmi2VariableHandle* var, fmi2Integer* length)
{
    TRACEFUNC
    if (length) {
        *length = var->vectorLength; 
    }
    return var->startVector;
}
...
fmi2Status fmi2_getVector(fmi2InstanceHandle* instance,
    const fmi2ValueReference valueReferences[],
    size_t nValueReferences,
    fmi2Vector values[])
{
    ...
    return instance->fmu->fmi2.getVector(instance->component,
        valueReferences,
        nValueReferences,
        values);
}
...
fmi2Status fmi2_setVector(fmi2InstanceHandle* instance,
    const fmi2ValueReference valueReferences[],
    size_t nValueReferences,
    const fmi2Vector values[], const fmi2Integer length[])
{
    TRACEFUNC

    return instance->fmu->fmi2.setVector(instance->component,
        valueReferences,
        nValueReferences,
        values,length);
}
...
fmi2Status fmi2_getVectorStatus(fmi2InstanceHandle* instance, const fmi2StatusKind s, fmi2Vector* value)
{
    TRACEFUNC
    return instance->fmu->fmi2.getVectorStatus(instance->component, s, value);
}
...
fmuHandle* fmi4c_loadUnzippedFmu_internal(const char* instanceName, const char* unzipLocation, bool unzippedLocationIsTemporary)
{
    ...
    fmu->fmi2.getVector = placeholder_fmi2_getVector;
    ...
    fmu->fmi2.setVector = placeholder_fmi2_setVector;
    ...
    fmu->fmi2.getVectorStatus = placeholder_fmi2_getVectorStatus;
