...
public:
        ...
        : desc_(path)
    {
        ...
        for (const auto& [name, component] : components) {
            auto model = NovaFmuLocator::resolve(desc_.dir(), component.source);
        ...
        for (const auto& connection : connections) {
            ...
            if (typeName == "Real") {
                ...
                make_connection<std::vector<double>>(source, sink);
            }
        }
...
std::unique_ptr<simulation_structure> nova_sim::load_ssp(const std::filesystem::path& path)
{
    if (!exists(path)) {
