#include "nova/components/algorithm/strong_coupling_algorithm.hpp"
...
struct strong_coupling_algorithm::impl {
    ...
    explicit impl(double dt, nova_engine& s) : baseStepSize(dt), sim(s) {}
};

strong_coupling_algorithm::strong_coupling_algorithm(double baseStepSize, nova_engine& sim)
    : pimpl_(std::make_unique<impl>(baseStepSize, sim)) {}

void strong_coupling_algorithm::initialize(double startTime) {}

double strong_coupling_algorithm::step(double currentTime, nova_engine& sim) {
