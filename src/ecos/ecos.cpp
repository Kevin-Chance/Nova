...
{
    std::unique_ptr<nova_sim::simulation> cpp_sim;
};

struct ecos_simulation_structure
{
    nova_sim::simulation_structure cpp_ss;
};
...
struct ecos_simulation_runner
{
    ...
    {
        cpp_runner = std::make_unique<nova_sim::simulation_runner>(*sim->cpp_sim);
    }
    std::unique_ptr<nova_sim::simulation_runner> cpp_runner;
};

struct ecos_simulation_listener
{
    std::unique_ptr<nova_sim::simulation_listener> cpp_listener;
};

struct ecos_parameter_set
{
    nova_sim::parameter_set cpp_parameter_set;
};
...
void ecos_set_log_level(const char* level)
{
    if (std::string("trace") == level) {
        set_logging_level(nova_sim::log::level::trace);
    } else if (std::string("debug") == level) {
        set_logging_level(nova_sim::log::level::debug);
    } else if (std::string("info") == level) {
        set_logging_level(nova_sim::log::level::info);
    } else if (std::string("warn") == level) {
        set_logging_level(nova_sim::log::level::warn);
    } else if (std::string("err") == level) {
        set_logging_level(nova_sim::log::level::err);
    } else if (std::string("off") == level) {
        set_logging_level(nova_sim::log::level::off);
    }
}
...
void ecos_simulation_structure_make_vector_connection(ecos_simulation_structure_t* ss, const char* source, const char* sink)
{
    try {
        ss->cpp_ss.make_connection<std::vector<double>>(source, sink);
    } catch (...) {
        handle_current_exception();
    }
}
...
{
    try {
        const auto ss = nova_sim::load_ssp(sspPath);

        auto algorithm = std::make_unique<nova_sim::fixed_step_algorithm>(stepSize);
...
ecos_simulation_listener_t* ecos_csv_writer_create(const char* resultFile, const char* csvConfig)
{
    try {
        ...
        auto writer = std::make_unique<nova_sim::csv_writer>(resultFile);
        if (csvConfig) {
...
bool ecos_csv_writer_set_decimation_factor(ecos_simulation_listener_t* writer, int decimationFactor)
{
    try {
        ...
        const auto csv_writer = dynamic_cast<nova_sim::csv_writer*>(writer->cpp_listener.get());
...
{
    nova_sim::plot_csv(csvFile, chartConfig);
}
...
{
    nova_sim::version v = nova_sim::library_version();
    return {v.major, v.minor, v.patch};
}

class my_listener : public nova_sim::simulation_listener
{
...
public:
    ...
    void pre_step(nova_sim::simulation& sim) override
    {
        if (preStepCallback_) preStepCallback_->operator()(createInfo(sim));
    }

    void post_step(nova_sim::simulation& sim) override
    {
...
private:
    ...
    static ecos_simulation_info createInfo(const nova_sim::simulation& sim)
    {
