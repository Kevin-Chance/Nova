...
本报告详尽列出了重构后的 `ecos_nova` 项目的架构，涵盖了所有命名空间、类及其方法。

## 1. 命名空间：`nova_sim` (核心仿真内核)

### 1.1 类：`simulation_structure`
**文件路径**：`include/ecos/structure/simulation_structure.hpp`
...
*   **公开方法**：
    ...
    *   `load(std::unique_ptr<algorithm> algorithm)`: 实例化并返回 `simulation` 对象。

### 1.2 类：`simulation`
**文件路径**：`include/ecos/simulation.hpp`
*   **功能**：仿真运行时的主控引擎，管理时间步进、数据交换和监听器通知。
*   **公开方法**：
    *   `simulation(std::unique_ptr<algorithm> algorithm)`: 构造函数。
    ...
    *   `add_listener(const std::string& name, std::shared_ptr<simulation_listener> listener)`: 注册仿真生命周期监听器。
...
### 1.3 类：`properties`
**文件路径**：`include/ecos/property.hpp`
*   **功能**：管理模型实例的变量映射。
*   **公开方法**：
...
**文件路径**：`include/ecos/nova_fmu_locator.hpp`
*   **公开方法**：
...
**文件路径**：`include/ecos/model_instance.hpp`
*   **公开方法**：
...
**头文件**：`include/ecos/nova_ecos.h`
*   **主要函数**：
    *   `nova_simulation_structure_create()` / `_destroy()`
    *   `nova_simulation_structure_add_model()`
    *   `nova_simulation_structure_make_connection()`
    *   `nova_simulation_create()` / `_destroy()`
    *   `nova_simulation_init()`
    *   `nova_simulation_step()`
    *   `nova_simulation_get_real() / _set_real()`
    *   `nova_simulation_add_csv_writer(sim, filename)`: 为 C 接口提供 CSV 记录支持。

---

## 4. 辅助工具与分布式逻辑

### 4.1 CSV 记录组件 (`nova_sim::csv_writer`)
**文件路径**：`include/ecos/listeners/csv_writer.hpp`
*   **`csv_config` 方法**：
...
*   **`csv_writer` 方法**：
    *   `explicit csv_writer(const std::string& filename)`: 构造函数。
    *   `config()`: 获取配置句柄。
    *   `post_init(simulation& sim)`: 仿真启动时写入表头。
    *   `post_step(simulation& sim)`: 每步步进后记录数值。
    *   `post_terminate(simulation& sim)`: 仿真结束时关闭文件。

### 4.2 日志系统 (`nova_sim::log`)
**文件路径**：`include/ecos/logger/logger.hpp`
*   **功能**：提供统一的格式化日志接口。
*   **核心方法**：
...
**文件路径**：`include/ecos/ssp/ssp_loader.hpp`
