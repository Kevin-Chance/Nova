...
public:
    ...
    void apply_sets()
    {
        ...
        for (const auto& p : vectorProperties_ | std::views::values) {
            p->applySet();
        }
    ...
    property_t<std::vector<double>>* get_vector_property(const std::string& name)
    {
        if (vectorProperties_.contains(name)) {
            const auto& property = vectorProperties_[name];
            return property.get();
        }
        return nullptr;
    }
    ...
    [[nodiscard]] const std::unordered_map<std::string, std::unique_ptr<property_t<std::vector<double>>>>& get_vectors() const
    {
        return vectorProperties_;
    }
   ...
   void add_vector_property(const std::string& name, std::unique_ptr<property_t<std::vector<double>>> p)
    {
        vectorProperties_[name] = std::move(p);
    }
    ...
    [[nodiscard]] std::vector<std::string> get_property_names() const
    {
        ...
        std::ranges::transform(vectorProperties_, std::back_inserter(names), [](auto& pair) {
            return pair.first;
        });
        return names;
    }
...
private:
    ...
    std::unordered_map<std::string, std::unique_ptr<property_t<std::vector<double>>>> vectorProperties_;
};
