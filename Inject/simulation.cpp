...
vector_connection* simulation::make_vector_connection(const variable_identifier& source, const variable_identifier& sink)
{
    const auto p1 = get_vector_property(source);
    if (!p1) throw std::runtime_error("No such vector property: " + source.str());
    const auto p2 = get_vector_property(sink);
    if (!p2) throw std::runtime_error("No such vector property: " + sink.str());

    pimpl_->connections_.emplace_back(std::make_unique<vector_connection>(p1, p2));
    return dynamic_cast<vector_connection*>(pimpl_->connections_.back().get());
}
...
property_t<std::vector<double>>* simulation::get_vector_property(const variable_identifier& identifier) const
{
    for (auto& instance : pimpl_->instances_) {
        if (instance->instanceName() == identifier.instanceName) {
            auto p = instance->get_properties().get_vector_property(identifier.variableName);
            if (p) return p;
        }
    }
    return nullptr;
