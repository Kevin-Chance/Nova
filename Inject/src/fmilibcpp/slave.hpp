...
public:
    ...
    std::vector<double> get_vector(value_ref vr, int len)
    {
        std::vector<std::vector<double>> values(1);
        get_vector({vr}, values, len);
        return values.front();
    }
    ...
    virtual bool get_vector(const std::vector<value_ref>& vrs, std::vector<std::vector<double>>& values, int len) = 0;
    ...
    virtual bool set_vector(const std::vector<value_ref>& vrs, const std::vector<std::vector<double>>& values) = 0;
