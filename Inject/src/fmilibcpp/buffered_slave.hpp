...
public:
    ...
    bool get_vector(
            const std::vector<value_ref>& vrs,
            std::vector<std::vector<double>>& values,
            int len) override
        {
            for (unsigned i = 0; i < vrs.size(); i++) {
                const value_ref vr = vrs[i];
                if (std::ranges::find(vectorToFetch_, vr) == vectorToFetch_.end()) {
                    mark_for_reading(get_model_description().get_by_vr<std::vector<double>>(vr)->name);
                }
                values[i] = vectorGetCache_.at(vr);
            }
            return true;
        }
    ...
    bool set_vector(const std::vector<value_ref>& vrs, const std::vector<std::vector<double>>& values) override
    {
        for (unsigned i = 0; i < vrs.size(); i++) {
            const value_ref vr = vrs[i];
            vectorSetCache_[vr] = values[i];
        }
        return true;
    }
    ...
    void transferCachedSets()
    {
        ...
        if (!vectorSetCache_.empty()) {
            std::vector<value_ref> vrs;
            std::vector<std::vector<double>> values;
            for (const auto& [vr, value] : vectorSetCache_) {
                vrs.emplace_back(vr);
                values.emplace_back(value);
            }
            slave_->set_vector(vrs, values);
            vectorSetCache_.clear();
        }
    }
    ...
    void receiveCachedGets()
    {
        ...
        if (!vectorToFetch_.empty()) {
            __vectorGetCache_.resize(vectorToFetch_.size());
            for (size_t i = 0; i < vectorToFetch_.size(); ++i) {
                const value_ref vr = vectorToFetch_[i];
                int len = vectorLengthCache_[vr];

                std::vector<std::vector<double>> tmp(1);
                slave_->get_vector(std::vector<value_ref>{vr}, tmp, len);
                __vectorGetCache_[i] = std::move(tmp[0]);
            }

            vectorGetCache_.clear();
            for (unsigned i = 0; i < vectorToFetch_.size(); i++) {
                const value_ref vr = vectorToFetch_[i];
                vectorGetCache_[vr] = __vectorGetCache_[i];
            }
        }
    }
    ...
    void mark_for_reading(const std::string& variableName)
    {
        ...
        if (v->is_integer()) {
            ...
            vectorToFetch_.emplace_back(vr);
        }
        ...
        if (initialized) {
            if (v->is_integer()) {
                ...
                vectorToFetch_.push_back(vr);
                vectorLengthCache_[vr] = v->vectorLength; 
                if (vectorGetCache_.find(vr) == vectorGetCache_.end()) {
                    vectorGetCache_[vr] = slave_->get_vector(vr, v->vectorLength);
                }
            }
        }
    }
...
private:
    ...
    std::unordered_map<value_ref, std::vector<double>> vectorSetCache_;
    ...
    std::unordered_map<value_ref, std::vector<double>> vectorGetCache_;
    ...
    std::vector<std::vector<double>> __vectorGetCache_;
    ...
    std::vector<value_ref> vectorToFetch_;

    std::unordered_map<value_ref, int> vectorLengthCache_;
