        ...
        , slave_(std::make_unique<fmilibcpp::buffered_slave>(std::move(slave)))
    {
        ...
        for (const auto& v : md.modelVariables) {
            ...
            if (v.is_integer()) {
                ...
                auto p = property_t<std::vector<double>>::create(
                    {slave_->instanceName, propertyName},
                    [&v, this] {
                        vrBuf[0] = v.vr;
                        slave_->get_vector(vrBuf, vBuf, v.vectorLength);
                        return vBuf.back(); 
                    },
                    [&v, this](const std::vector<double>& value) {
                        vrBuf[0] = v.vr;
                        vBuf[0] = value;
                        slave_->set_vector(vrBuf, vBuf);
                        return vBuf.back();
                    });
                properties_.add_vector_property(propertyName, std::move(p));
            }
            else {
